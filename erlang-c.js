(function (global, factory) {
  ...
  (global = global || self, factory(global.ErlangC = {}));
  ...
  var Decimal = clone(DEFAULTS); // Create the internal constants from their string values.

  LN10 = new Decimal(LN10);
  PI = new Decimal(PI);

  var factorialCache = [new Decimal("1"), new Decimal("1")];
  var getFactorial = function getFactorial(n) {
    ...
    var result = factorialCache[i - 1];

    for (; i <= n; i++) {
      factorialCache[i] = result = result.times(i.toString());
    }
  ...
  getFactorial(100);
  var getIntensity = function getIntensity(volume, aht, intervalLength) {
    return volume * (aht / intervalLength);
  };
  var getAN = function getAN(intensity, agents) {
    return new Decimal(intensity).toPower(agents);
  };
  var getX = function getX(AN, factorial, intensity, agents) {
    return AN.dividedBy(factorial).times(agents / (agents - intensity));
  };
  var getY = function getY(intensity, agents) {
    var sum = new Decimal(0);

    for (var i = 0; i < agents; i++) {
      var iFact = getFactorial(i);
      var aPowI = new Decimal(intensity).toPower(i);
      sum = sum.plus(aPowI.dividedBy(iFact));
    }

    return sum;
  };
  var getPW = function getPW(X, Y) {
    return X.dividedBy(Y.plus(X));
  };
  var getErlangC = function getErlangC(AN, factorial, intensity, agents) {
    var X = getX(AN, factorial, intensity, agents);
    var Y = getY(intensity, agents);
    var PW = getPW(X, Y);
    return PW;
  };
  var getServiceLevel = function getServiceLevel(erlangC, intensity, agents, targetTime, aht) {
    return 1 - erlangC.times(Math.exp(-(agents - intensity) * (targetTime / aht)));
  };
  var getFullServiceLevel = function getFullServiceLevel(intensity, agents, targetTime, aht) {
    var factorial = getFactorial(agents);
    var AN = getAN(intensity, agents);
    var erlangC = getErlangC(AN, factorial, intensity, agents);
    var serviceLevel = getServiceLevel(erlangC, intensity, agents, targetTime, aht);
    return serviceLevel;
  };
  var checkMaxOccupancy = function checkMaxOccupancy(intensity, agents, maxOccupancy) {
    var occupancy = intensity / agents;
  ...
  var getNumberOfAgents = function getNumberOfAgents(volume, intervalLength, aht, targetServiceLevel, targetTime, maxOccupancy, shrinkage) {
    var intensity = getIntensity(volume, aht, intervalLength);
    var agents = Math.floor(intensity) + 1;

    while (getFullServiceLevel(intensity, agents, targetTime, aht) < targetServiceLevel) {
      agents += 1;
    }
  ...
  exports.checkMaxOccupancy = checkMaxOccupancy;
  exports.getAN = getAN;
  exports.getErlangC = getErlangC;
  exports.getFactorial = getFactorial;
  exports.getFullServiceLevel = getFullServiceLevel;
  exports.getIntensity = getIntensity;
  exports.getNumberOfAgents = getNumberOfAgents;
  exports.getPW = getPW;
  exports.getServiceLevel = getServiceLevel;
  exports.getX = getX;
  exports.getY = getY;
